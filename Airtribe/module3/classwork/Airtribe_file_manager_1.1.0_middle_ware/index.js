...
function readSyncWriteSync() {
  ...
  console.log(data);
  fs.writeFileSync("./AFM-destination/output_Sync_read_write_Sync.txt", data, {
    encoding: "utf8",
    flag: "w",
  });
...
function readFileSync_writeFileAsync() {
  ...
  console.log(data);
  fs.writeFile(
    "./AFM-destination/output_Sync_read_write_aSync.txt",
    data,
    { encoding: "utf8", flag: "w" },
    function (err, data) {
      if (err) {
        console.log("data had not syn");
      } else {
        console.log("synced");
      }
    }
...
function read_Async_write_Async() {
  const data = fs.readFile(
    ...
    function (err, data) {
      ...
      console.log(r);
      fs.writeFile(
        "./AFM-destination/output_ASync_read_write_ASync.txt",
        r,
        { encoding: "utf8", flag: "w" },
        function (err, data) {
          if (err) {
